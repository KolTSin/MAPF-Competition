...
Plan PlanMerger::merge_agent_plans(const std::vector<Plan>& agent_plans,
                                   int num_agents) {
    ...
    for (std::size_t t = 0; t < horizon; ++t) {
        ...
        for (int agent = 0; agent < num_agents; ++agent) {
            if (t < agent_plans[agent].size()) {
                std::cerr << agent_plans[agent].steps[t].actions[agent].to_string() << '\n';
                ja.actions[agent] = agent_plans[agent].steps[t].actions[agent];
            }
        }
